...
enum NvmeAdminCommands {
    ...
    NVME_ADM_CMD_FEMU_RECOVER   = 0xc2,
};
...
#define femu_debug(fmt, ...) \
    do { printf("[FEMU] Dbg: " fmt "\r", ## __VA_ARGS__); } while (0)
#else
#define femu_debug(fmt, ...) \
    do { } while (0)
#endif
...
#define femu_err(fmt, ...) \
    do { fprintf(stderr, "[FEMU] Err: " fmt "\r", ## __VA_ARGS__); } while (0)

#define femu_log(fmt, ...) \
    do { printf("[FEMU] Log: " fmt "\r", ## __VA_ARGS__); } while (0)
